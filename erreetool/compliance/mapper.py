...
class ComplianceMapper:
    ...
    def classify_finding(self, finding: str) -> str:
        ...
        if re.search(r"cve-\d{4}-\d{4,7}", finding_lower):
            if any(
                kw in finding_lower
                for kw in ["critical", "rce", "remote code execution"]
            ):
                return "cve_critical"
            return "cve_high"

        # Network exposure
        if "smb" in finding_lower and (
            "445" in finding_lower
            or "exposed" in finding_lower
            or "open" in finding_lower
        ):
            return "smb_exposed"
        if "rdp" in finding_lower and (
            "3389" in finding_lower
            or "exposed" in finding_lower
            or "open" in finding_lower
        ):
            return "rdp_exposed"
        if "ssh" in finding_lower and (
            "22" in finding_lower
            or "exposed" in finding_lower
            or "open" in finding_lower
        ):
        ...
        if any(
            db in finding_lower
            for db in ["mysql", "postgresql", "mssql", "oracle", "mongodb", "redis"]
        ) and ("exposed" in finding_lower or "open" in finding_lower):
            return "database_exposed"
        if (
            "80/tcp" in finding_lower
            or "443/tcp" in finding_lower
            or "8080" in finding_lower
            or "web server" in finding_lower
        ):
            return "web_exposed"
        ...
        # Auth findings
        if (
            "default cred" in finding_lower
            or "default password" in finding_lower
            or "default login" in finding_lower
        ):
            return "default_credentials"
        if "weak password" in finding_lower or "password policy" in finding_lower:
            return "weak_password_policy"
        if "mfa" in finding_lower and (
            "missing" in finding_lower
            or "not enabled" in finding_lower
            or "disabled" in finding_lower
        ):
            return "mfa_missing"
        ...
        # Data protection
        if (
            "sensitive data" in finding_lower
            or "pii" in finding_lower
            or "credit card" in finding_lower
            or "ssn" in finding_lower
        ):
            return "sensitive_data_exposure"
        if "encryption" in finding_lower and (
            "missing" in finding_lower
            or "not enabled" in finding_lower
            or "disabled" in finding_lower
        ):
        ...
        if (
            "kerberos" in finding_lower
            or "as-rep" in finding_lower
            or "as_rep" in finding_lower
        ):
            return "kerberos_issues"

        # Cloud
        if "s3" in finding_lower and (
            "public" in finding_lower or "open" in finding_lower
        ):
            return "s3_public"
        if "iam" in finding_lower and (
            "overprivileged" in finding_lower
            or "excessive" in finding_lower
            or "wildcard" in finding_lower
        ):
        ...
        if "docker api" in finding_lower and (
            "exposed" in finding_lower or "open" in finding_lower
        ):
    ...
    def _export_markdown(self, report: ComplianceReport) -> str:
        ...
        for mapping in report.mappings:
            ...
            for m in mapping.mappings:
                lines.append(
                    f"- **{m['framework'].upper()} Controls:** {', '.join(m['controls'])}"
                )
