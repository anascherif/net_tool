...
from erreetool.agent.memory import memory_store, memory_retriever, MemoryType

console = Console()


def run(
    subcommand: str = typer.Argument("stats", help="Subcommand: list, show, search, stats, clear, export, import"),
    ...
    output: str = typer.Option("memory_export.json", "--output", "-o", help="Output file for export"),
    input_file: str = typer.Option(None, "--input", "-i", help="Input file for import"),
...
def _list_memory(type: str = None, limit: int = 20, tag: str = None):
    """List memory entries."""
    if type:
        try:
...
def _show_memory(entry_id: str = None, raw: bool = False):
...
def _search_memory(query: str = None, type: str = None, limit: int = 20):
...
def _clear_memory(type: str = None, confirm: bool = False):
...
def _export_memory(output: str = "memory_export.json", type: str = None):
    """Export memory to JSON file."""
    mem_type = MemoryType(type) if type else None
    entries = memory_store.get_by_type(mem_type) if mem_type else list(memory_store._entries.values())
...
def _import_memory(input_file: str = None, merge: bool = True):
